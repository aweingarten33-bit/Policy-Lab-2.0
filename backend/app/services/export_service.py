...
def _counsel_phrase(industry: Optional[str]) -> str:
    ...
    "Consult qualified healthcare compliance counsel" was hardcoded, so an
    industrial OSHA noise policy for a manufacturing plant ended with advice
    to consult a healthcare lawyer.
    """
    return {
...
def _handles_phi(industry: Optional[str]) -> bool:
    """Whether a PHI statement belongs in this document at all."""
    return (industry or "") in ("healthcare", "home_health", "pharmacy")


def _add_disclaimer_box(doc, industry: Optional[str] = None, document_kind: str = "report"):
    ...
    # Written for whichever document this is. Calling a rewritten policy "this
    # report" made the closing page read as though it belonged to a different
    # document -- which, to a prospect, is exactly what it looks like.
    kind = document_kind if document_kind in ("report", "policy") else "report"
    this_doc = "This report" if kind == "report" else "This policy"

    disclaimers = [
        ("Source Attribution & Verification:", f"{this_doc} uses a 4-tier source labeling system. Claims labeled 'Verified Source' have been cross-checked against source material. Claims labeled 'Retrieved Source' come from the curated knowledge base. Claims labeled 'Live Research' come from controlled web searches of curated regulatory sites. Claims labeled 'Model Inference' come from the AI model's training data and have NOT been verified against source material — they require independent review by qualified compliance counsel before being acted upon."),
        ("AI Hallucination Warning:", f"{this_doc} was generated by an AI model and may contain inaccuracies, including fabricated regulatory citations or misinterpreted requirements. Every finding, citation, and suggested change MUST be independently verified against the source regulation before being acted upon. Pay special attention to claims labeled 'Model Inference' or 'Unverified'."),
        ("Not Legal Advice:", f"This document does not constitute legal advice and does not create an attorney-client relationship. It is intended as a starting point for internal review only. Consult {_counsel_phrase(industry)} before relying on any content herein."),
    ]

    # A PHI statement on a factory noise policy is noise itself, and it signals
...
def _build_cover_page(doc: Document, package: ComplianceActionPackage, file_name: Optional[str]):
    ...
    _add_disclaimer_box(doc, industry=getattr(package, "industry", None), document_kind="report")
    doc.add_page_break()
...
def generate_draft_policy_docx(policy: dict) -> bytes:
    ...
    # AI disclaimer
    doc.add_paragraph()
    _add_disclaimer_box(doc, industry=policy.get("industry"), document_kind="policy")
...
def generate_updated_policy_docx(rewritten: RewrittenPolicy) -> bytes:
    ...
    doc.add_paragraph()
    _add_disclaimer_box(doc, industry=getattr(rewritten, "industry", None), document_kind="policy")
