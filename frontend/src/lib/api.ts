...
/**
 * Streaming version — calls /api/action-package-stream and calls onUpdate
 * each time a phase completes (gap analysis first, then parallel batches).
 * Resolves with the final complete package.
 */
export async function generateActionPackageStream(
  text: string,
  ...
  const response = await fetch(`${API_BASE}/api/action-package-stream`, {
    method: "POST",
  ...
  while (true) {
    ...
    for (const part of parts) {
      ...
      try {
        ...
        lastPkg = data as ComplianceActionPackage;
        onUpdate(lastPkg);
      } catch (e) {
...
export async function generateActionPackage(
  ...
  const response = await fetch(`${API_BASE}/api/action-package`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      text,
...
/**
 * Upload a policy file and generate the action package.
 */
export async function generateActionPackageFromFile(
  file: File,
  jurisdiction?: string,
): Promise<ComplianceActionPackage> {
  ...
  const response = await fetch(`${API_BASE}/api/action-package-file`, {
    method: "POST",
    body: formData,
  });
...
/**
 * Legacy: Analyze policy text against applicable regulations (single output).
 */
...
/**
 * Legacy: Upload a policy file for gap analysis.
 */
...
/**
 * Export the complete action package as a .docx file.
 */
export async function exportActionPackage(
  ...
  let downloadName = "Compliance_Action_Package.docx";
...
/**
 * Legacy: Export a single gap analysis report.
 */
export async function exportReport(
  ...
  const blob = await response.blob();
  const contentDisposition = response.headers.get("Content-Disposition");
  let downloadName = "Gap_Report.docx";
  if (contentDisposition) {
    const match = contentDisposition.match(/filename="?(.+?)"?$/);
    if (match) downloadName = match[1];
  }
...
/**
 * Copy analysis results to clipboard as formatted text.
 */
export function copyToClipboard(result: AnalysisResult, fileName: string) {
  const lines = [
    `POLICY GAP ANALYSIS — ${result.policy_type}`,
    `Generated: ${new Date().toLocaleDateString()}  |  File: ${fileName || "Uploaded Policy"}`,
    ``,
    `Critical: ${result.critical_count}  |  Gaps: ${result.gap_count}  |  Partial: ${result.partial_count}  |  Compliant: ${result.compliant_count}`,
    ``,
    `REGULATIONS CHECKED:`,
    result.regulations_applied.join(", "),
    ``,
    `PRIORITY FINDINGS:`,
    ...result.priority_findings.map((f, i) => `${i + 1}. ${f}`),
    ``,
    `GAP TABLE:`,
    ...result.gap_table.map(
      (r) =>
        `[${(r.status || "").toUpperCase()} | ${r.risk_level?.toUpperCase() || ""}] ${r.clause}\n  Finding: ${r.finding}\n  Suggested: ${r.suggested_language}`
    ),
    ``,
    `EXECUTIVE SUMMARY:`,
    result.audit_ready_summary,
  ].join("\n");
...
/**
 * Draft a brand-new policy from a plain-English description.
 */
export async function draftPolicy(
  ...
  const response = await fetch(`${API_BASE}/api/draft-policy`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
/**
 * Download a compliance certificate for a completed analysis.
 */
export async function exportCertificate(
  pkg: ComplianceActionPackage,
  fileName?: string,
): Promise<void> {
  const response = await fetch(`${API_BASE}/api/export-certificate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ package: pkg, file_name: fileName }),
  });

  if (!response.ok) {
    const errorData = await response.json().catch(() => ({ detail: "Certificate export failed" }));
    throw new Error(errorData.detail || `Certificate export failed (${response.status})`);
  }

  const blob = await response.blob();
  const contentDisposition = response.headers.get("Content-Disposition");
  let downloadName = "Compliance_Certificate.docx";
  if (contentDisposition) {
    const match = contentDisposition.match(/filename="?(.+?)"?$/);
    if (match) downloadName = match[1];
  }
